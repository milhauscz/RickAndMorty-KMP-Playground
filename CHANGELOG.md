...
First release shipping feature modules directly — no `:sdk` facade.
...
- Domain models, `Result<D, DataError>`, repositories, and use cases as the public vocabulary — no
  wrapper types.
...
- Removed `:sdk`, `:sdk:compose`, and `samples/consumer`. `:shared` is the integration demo.
